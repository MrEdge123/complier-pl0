...
注意到，在生成`jpc`指令的时候，因为是从上到下分析的，这时还不知道**跳转的指令地址**。所以，只能当知道指令地址的时候，再**回填**指令地址，`jpc`指令才算完整
...
最后，生成完`jmp`指令后，还要**回填**`jpc`指令，所以第3部分流程是：
